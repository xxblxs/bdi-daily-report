...
执行后在 `./reports/` 生成 HTML 文件，同时推送到企业微信群。
...
编辑 `.github/workflows/bdi_daily.yml`，修改 cron 表达式（UTC 时间，北京时间 -8 小时）：
...
3. 创建完成后复制 Webhook 地址：
   ...
   ```
4. 填入 `.env` 的 `WECOM_WEBHOOK`，或更新 GitHub Secret

> 修改 webhook 后用 gh CLI 更新 Secret（无需改代码）：
> ```bash
> gh secret set WECOM_WEBHOOK --repo 你的用户名/仓库名 --body "新的webhook地址"
> ```
...
4. 更新 GitHub Secret：
   ```bash
   gh secret set NAVGREEN_TOKEN --repo 你的用户名/仓库名 --body "你的token"
   ```
...
**Q: 如何查看历史报告？**
A: 每次 Actions 执行后，HTML 报告会上传为 Artifact，在 Actions 运行记录页面可下载，保留 30 天。
